...
const TaskCard = ({ task, onStatusChange, onDelete }) => {
  ...
  return (
    <div
      className={`group bg-zinc-900 border rounded-xl p-4 transition-all duration-200
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <button
            onClick={() =>
              onStatusChange(task._id, isPending ? "completed" : "pending")
            }
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span
            className={`text-xs px-2 py-0.5 rounded-full border font-medium ${STATUS_STYLES[task.status]}`}
          >
            {task.status}
          </span>
          <button
